...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; color: string }> = {
    ...
    in_transit:        { label: 'Em trânsito',    color: 'bg-indigo-100 text-indigo-800' },
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-gray-500 text-sm">Carregando informações do pedido...</p>
      </div>
    </div>
  )
  ...
  if (error) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="text-center max-w-md">
        ...
        <p className="text-gray-500 text-sm">Verifique o código de rastreamento e tente novamente.</p>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-black text-white py-4 px-6">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          <span className="text-gray-400 text-xs">Sistema de Acompanhamento</span>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="bg-black px-6 py-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-gray-400 text-xs mb-1">Código de rastreamento</p>
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            {lastEvent && (
              <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
                <p className="text-xs text-blue-500 font-medium mb-1">Última atualização</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 px-6 py-5">
          <h2 className="text-sm font-semibold text-gray-900 mb-6">Histórico de atualizações</h2>

          {tracking.events.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-400 text-sm">Nenhuma atualização ainda.</p>
              <p className="text-gray-300 text-xs mt-1">As atualizações aparecerão aqui em breve.</p>
            </div>
          ) : (
        ...
        <p className="text-center text-xs text-gray-400 pb-4">
          Acompanhamento disponível até {formatDate(tracking.expiresAt)} · {order.trackingId}
        </p>
      </div>
    </div>
  )
