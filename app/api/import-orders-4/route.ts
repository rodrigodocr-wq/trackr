...
const SHOP_DOMAIN = 'ez1exp-4e.myshopify.com'

const ORDERS = [
  { number: '1561', shopifyId: '6634920706161', name: 'Jason Purcell', email: 'jasonpurcell3328@gmail.com', product: 'Anti-Snoring Mouthguard', address: 'GB', created: '2026-06-02T12:00:00+00:00' },
]
...
export async function GET(req: NextRequest) {
  ...
  if (!store) {
    const { data: ns } = await supabase.from('stores').insert({ shop_domain: SHOP_DOMAIN, access_token: '', name: 'Navaro UK' }).select('id').single()
    store = ns
  }
